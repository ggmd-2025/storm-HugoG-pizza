...
public class Exit3Bolt extends BaseRichBolt {
    ...
    public void prepare(Map stormConf, TopologyContext context, OutputCollector collector) {
        ...
        try {
            // Ouverture du port immédiate
            System.out.println("Exit3Bolt: Opening server socket on port " + port);
            this.serverSocket = new ServerSocket(port);
            
            new Thread(() -> {
                try {
                    System.out.println("Exit3Bolt: Waiting for client...");
                    Socket clientSocket = serverSocket.accept();
                    System.out.println("Exit3Bolt: Client connected!");
    ...
    public void execute(Tuple input) {
        try {
            ...
            String rang = input.getStringByField("rang"); // Champ spécifique T3
