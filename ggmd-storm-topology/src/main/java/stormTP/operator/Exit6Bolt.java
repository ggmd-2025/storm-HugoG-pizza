...
public class Exit6Bolt extends BaseRichBolt {
    ...
    public void prepare(Map stormConf, TopologyContext context, OutputCollector collector) {
        ...
        try {
            System.out.println("Exit6Bolt: Opening server socket on port " + port);
            this.serverSocket = new ServerSocket(port);
            new Thread(() -> {
                try {
                    System.out.println("Exit6Bolt: Waiting for client...");
    ...
    public void execute(Tuple input) {
        try {
            ...
            String evolution = input.getStringByField("evolution");

            JsonObjectBuilder builder = Json.createObjectBuilder();
            builder.add("id", id);
            builder.add("nom", nom);
            builder.add("date", date);
            builder.add("evolution", evolution);
